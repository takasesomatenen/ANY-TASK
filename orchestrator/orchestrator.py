...
必要な環境変数:
  ...
  GOOGLE_API_KEY      ... Gemini API キー (Google AI Studio / 任意)
...
def orchestrate(task: str) -> dict:
    log("→ Gemini と GPT に並列で問い合わせ中...")
    started = time.monotonic()
    with ThreadPoolExecutor(max_workers=2) as pool:
        f_gemini = pool.submit(call_gemini, task)
        f_gpt = pool.submit(call_gpt, task)
        gemini_out = f_gemini.result()
        gpt_out = f_gpt.result()
    log(f"   ワーカー完了 ({time.monotonic() - started:.1f}秒)")

    log(f"→ Claude ({CLAUDE_MODEL}) がレビュー・統合中...")
    review_prompt = build_review_prompt(task, gemini_out, gpt_out)
    final = call_claude(review_prompt)
    return {
        "task": task,
        "gemini": gemini_out,
        "gpt": gpt_out,
        "claude_review": final,
        "models": {"claude": CLAUDE_MODEL, "gemini": GEMINI_MODEL, "gpt": GPT_MODEL},
...
モデル: Claude={m.get('claude', '?')} / Gemini={m.get('gemini', '?')} / GPT={m.get('gpt', '?')}
